...
public:
	void Initialize();
	void PressPowerButton(unsigned long duration = 500);
	void Update();

	bool IsPowerOn() const { return m_isPowerOn; }

private:
	void UpdatePowerButton();
	void UpdatePowerCheck();

	unsigned long m_pressTimestamp = 0;
	unsigned long m_targetPressDuration = 0;
	bool m_pressRequested = false;
